# -*- coding: utf-8 -*-
"""
维普论文 PDF 可下载性统计工具 - 端口9223
win + r chrome.exe --remote-debugging-port=9223 --user-data-dir="C:\chrome_debug_profile2"
...
# -- 拟人化延迟配置 --
PAGE_SLEEP          = (3.0, 5.0)   # 每页停留+翻页统一间隔
...
async def process_page(page, conn, page_num, institution_name):
    ...
    # 拟人化停留
    sleep_s = random.uniform(*PAGE_SLEEP)
    print(f"  ...等待 {sleep_s:.1f}s")
...
async def main():
    ...
    async with async_playwright() as p:
        try:
            ...
            print(f"      请确认 Chrome 已用 --remote-debugging-port=9223 启动")
            print(f"      {e}")
            conn.close()
            sys.exit(1)
